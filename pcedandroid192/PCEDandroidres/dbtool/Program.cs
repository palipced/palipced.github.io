...
namespace makedict
{
    class Program
    {
        ...
        //替换cidian中所有的汉字、符号加上空格
        public static void tihuanhanzi()
        {
            string strLine;

            StreamReader sr = new StreamReader(new FileStream(@".\xxxcidian", FileMode.Open), System.Text.Encoding.GetEncoding("utf-8"));
            StreamWriter sw = new StreamWriter(@".\xxxcidianspace.txt", false, System.Text.Encoding.GetEncoding("utf-8"));
            ...
            {
                /*/日文
                strLine = new Regex(@"[\u0800-\u4e00]", RegexOptions.None).Replace(strLine, "");
                //英文与数字
                strLine = new Regex(@"[A-Za-z0-9]", RegexOptions.None).Replace(strLine, "");
                //巴利字母
                strLine = new Regex(@"[āīūṅñṭḍṇḷŋĀĪŪṄÑṬḌṆḶŊṁṃṀṂ]", RegexOptions.None).Replace(strLine, "");
                //缅文
                strLine = new Regex(@"[\u1000-\u109f|\uaa60-\uaa7f]", RegexOptions.None).Replace(strLine, "");
                //89个越南字母可能不全并且里面还有其它字母
                strLine = new Regex("êûâü̊άçίśđưìóĩôàăáē¿ŚơúũãòèéöýùíäõρέïîÂόōήµžǡðëšζǔĐμẠ́̇νÔœæώ̄ƶοůåÄ̈́εκαδιĝÆğηǙÛčÝªþćėͅŕŭ̦ßÐķʨɕʕәς", RegexOptions.None).Replace(strLine, "");
                */

                //汉字
                strLine = new Regex(@"[\u4e00-\u9fa5]", RegexOptions.None).Replace(strLine, delegate (Match m) { return " " + m.Value + " "; });
                //符号
                strLine = new Regex(@"[\[\+\]%:\(\)\<\>/\-,. " + "\"" + "\t" + @"¼﹕Ｐˋ×¾ˇ½̆÷{／`＜˚．％̃％λ" + @" ﹔～－¤＝#ˊ·﹝﹞}&\?§！？_\\︰\*" + "«»=;：﹑﹐，（。）~；'°!＋]", RegexOptions.None).Replace(strLine, delegate (Match m) { return " " + m.Value + " "; });
                strLine = new Regex(" {2,}", RegexOptions.None).Replace(strLine, " ");
        ...
        //从文本取得所有的越南字母
        public static void getyuenanabc()
        {
            StreamReader sr = new StreamReader(new FileStream(@".\xxxabc.txt", FileMode.Open), System.Text.Encoding.GetEncoding("utf-8"));
            StreamWriter sw = new StreamWriter(@".\abcyuenan.txt", false, System.Text.Encoding.GetEncoding("utf-8"));
        ...
        //取得217本经典书文档文件名与目录书名的对应关系
        public static void save_name_to_file()
        {
            int j = 0;
            ...
            //ID NNAME filename PARENTID ISLEAF DISPLAY_ORDER
            string ID = "";
            string NNAME = "";
            string filename = "";
            string PARENTID = "";
            ...
            StreamReader sr = new StreamReader(new FileStream(@".\mulu.txt", FileMode.Open), System.Text.Encoding.GetEncoding(65001));
            string strLine = sr.ReadLine();
            while (strLine != null)
            {
                //^(?<w0>\b\w{8,20}\b)"(?<w1>.*?)", "(?<w2>.*?)", "(?<w3>.*?)", "(?<w4>.*?)", "(?<w5>.*?)", (?<w6>.*?);
                MatchCollection mc = new Regex("^(?<w0>\\b\\w{8,20}\\b)\"(?<w1>.*?)\", \"(?<w2>.*?)\", \"(?<w3>.*?)\", \"(?<w4>.*?)\", \"(?<w5>.*?)\", (?<w6>.*?);", RegexOptions.None).Matches(strLine);
                if (mc.Count == 1)
                {
                    foreach (Match ma in mc)
                    {
                        biaozhi = ma.Groups["w0"].Value;

                        ID = ma.Groups["w1"].Value;

                        NNAME = ma.Groups["w2"].Value;

                        filename = ma.Groups["w3"].Value;

                        PARENTID = ma.Groups["w4"].Value;

                        ISLEAF = ma.Groups["w5"].Value;

                        DISPLAY_ORDER = ma.Groups["w6"].Value;
                        ...
                        if (ISLEAF == "1")
                        {
                            BOOK = NNAME;
                            //叶子节点只可能是第三或第四级缩进
                            //如果叶子是第三级缩进，表明其上只有两层目录，第三层目录记为空格
                            //否则叶子只能是第四级缩进，其上是三层目录且目录已赋值，
                            if (biaozhi== "nnnnnnnnnnnnnnnn") {
                                C = " ";
                            }
                            StreamWriter sw = new StreamWriter(@".\test.txt", true, System.Text.Encoding.GetEncoding(65001));
                            sw.WriteLine(filename + "#" + A + "#" + B + "#" + C + "#" + BOOK);
                            sw.Close();
                        }
                        else {
                            if (biaozhi == "aaaaaaaa")
                        ...
                        //StreamWriter sw = new StreamWriter(@".\test.txt", true, System.Text.Encoding.GetEncoding(65001));
                        //sw.WriteLine(""+ biaozhi + "\""+ ID + "\", \""+ NNAME + "\", \""+ filename + "\", \""+ PARENTID + "\", \""+ ISLEAF + "\", "+ DISPLAY_ORDER + ";");
                        //sw.Close();
        ...
        public static void dbtool()
        {
            //SQLiteConnection.CreateFile(@".\testDB.db");
            SQLiteConnection dbConnection = new SQLiteConnection(@"data source = .\testDB.db");
            dbConnection.Open();
            ...
            dbCommand.CommandText = "CREATE VIRTUAL TABLE IF NOT EXISTS canons USING fts4(ID INTEGER,Email TEXT);";
            dbCommand.ExecuteReader().Close();
            dbCommand.CommandText = "insert into canons (ID, Email) values('1','1231 10086 abc巴利āīūṅñṭḍṇḷṃṁŋ123 123');";
            dbCommand.ExecuteReader().Close();
            dbCommand.CommandText = "insert into canons (ID, Email) values('2','1232 10086 abc巴利āīūṅñṭḍṇḷṃṁŋ123 \r\n\r\n 123');";
            dbCommand.ExecuteReader().Close();
            dbCommand.CommandText = "insert into canons (ID, Email) values('3清净','1233 abc巴利āīūṅñṭḍṇḷṃṁŋ123 10086 123');";
            dbCommand.ExecuteReader().Close();

            dbCommand.CommandText = "SELECT * FROM canons WHERE Email MATCH 'abc巴利āīūṅñṭḍṇḷṃṁŋ123 1232 10086 123';";
        ...
        public static void dancipx()
        {
            ...
            StreamWriter sw = new StreamWriter(@".\dancipx.txt", false, System.Text.Encoding.GetEncoding("utf-8"));
            for (int k = 0; k < NUM; k++)
            {
