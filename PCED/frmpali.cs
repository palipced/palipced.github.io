...
namespace pced
{
    ...
    public partial class frmpali : Form, IMessageFilter
    {
        ...
        //1110
        /// <summary>
        /// 本窗口所显示经文的文件名，不带.htm后缀
        /// </summary>
        public string htmfilename = "";
        //
        ...
        /// 本窗口经文对应父窗口的初始章节标记
        /// </summary>
        public string selfchaptermark = "";
        /// <summary>
        /// 父窗口经文的段号，本窗口刚打开时需要和它相对应
        /// </summary>
        public string selfparanum = "";
        //

        //2021.10.28 用来储存当前所显示的段落所对应的义注等书籍的文件全名/章节标记/记下段落号
        public string tsbtnmula_htmfn="";
        public string tsbtnattha_htmfn = "";
        public string tsbtntika_htmfn = "";

        public string tsbtnmula_zj = "";
        public string tsbtnattha_zj = "";
        public string tsbtntika_zj = "";

        public string tsbtnmula_paranum = "";
        public string tsbtnattha_paranum = "";
        public string tsbtntika_paranum = "";
        //
        ...
        /// <summary>
        /// 标记本窗口是从目录还是从“根本、义注、复注”三个按钮所打开的：默认0表示从目录，当从目录打开时不需要进行义注跳转定位
        /// </summary>
        public int isfromMAT = 0;
            ...
            else
            {
                ...
                foreach (string strName in Program.mainform.alName)
                {
                    if (strName.Substring(0, 20).Trim() == mulafile)
                    {
                        ...
                        frmw.Text = trNx.Text;
                        //2021.10.13为完善义注连接，改动读经窗口里的代码，三个按钮计三处。后注：其实不用
                        frmw.htmfilename = trNx.Name;
                        //
        ...
        */
        ...
        //tsbtnmula_htmfn    tsbtnmula_zj    tsbtnmula_paranum
        //tsbtnattha_htmfn    tsbtnattha_zj    tsbtnattha_paranum
        //tsbtntika_htmfn    tsbtntika_zj    tsbtntika_paranum
        private void tsbtnmula_Click(object sender, EventArgs e)
        {
            string bookpath = @".\pali\" + tsbtnmula_htmfn;

            if (!(File.Exists(bookpath)))
            {
                MessageBox.Show("此篇经典文件没找到！可能是文件不全或者是删除了文件！");
                return;
            }
            ...
            foreach (string strName in Program.mainform.alName)
            {
                if (strName.Substring(0, 20).Trim()+".htm"== tsbtnmula_htmfn)
                {
                    TreeNode trNx;
                    string sNodeIdx = strName.Substring(20);
                    trNx = Program.mainform.frmmuluwindow.treeView2.Nodes[Convert.ToInt32(sNodeIdx.Substring(sNodeIdx.Length - 1 * 2, 1 * 2).Trim())];
                    ...
                    frmw.Text = trNx.Text;
                    //2021.10.13为完善义注连接，改动读经窗口里的代码，三个按钮计三处。
                    frmw.htmfilename = trNx.Name;
                    frmw.selfchaptermark = tsbtnmula_zj;
                    frmw.selfparanum = tsbtnmula_paranum;
                    frmw.isfromMAT = 1;
                    //
                    frmw.mulafile = ((tvtag)(trNx.Tag)).fnmula;
                    frmw.atthafile = ((tvtag)(trNx.Tag)).fnattha;
        ...
        private void tsbtnattha_Click(object sender, EventArgs e)
        {
            string bookpath = @".\pali\" + tsbtnattha_htmfn;
            ...
            foreach (string strName in Program.mainform.alName)
            {
                if (strName.Substring(0, 20).Trim()+".htm" == tsbtnattha_htmfn)
                {
                    TreeNode trNx;
                    string sNodeIdx = strName.Substring(20);
                    trNx = Program.mainform.frmmuluwindow.treeView2.Nodes[Convert.ToInt32(sNodeIdx.Substring(sNodeIdx.Length - 1 * 2, 1 * 2).Trim())];
                    ...
                    //2021.10.13为完善义注连接，改动读经窗口里的代码，三个按钮计三处。
                    frmw.htmfilename = trNx.Name;
                    frmw.selfchaptermark = tsbtnattha_zj;
                    frmw.selfparanum = tsbtnattha_paranum;
                    frmw.isfromMAT = 1;
                    //
                    frmw.mulafile = ((tvtag)(trNx.Tag)).fnmula;
                    frmw.atthafile = ((tvtag)(trNx.Tag)).fnattha;
        ...
        private void tsbtntika_Click(object sender, EventArgs e)
        {
            string bookpath = @".\pali\" + tsbtntika_htmfn;
            ...
            foreach (string strName in Program.mainform.alName)
            {
                if (strName.Substring(0, 20).Trim()+".htm" == tsbtntika_htmfn)
                {
                    TreeNode trNx;
                    string sNodeIdx = strName.Substring(20);
                    trNx = Program.mainform.frmmuluwindow.treeView2.Nodes[Convert.ToInt32(sNodeIdx.Substring(sNodeIdx.Length - 1 * 2, 1 * 2).Trim())];
                    ...
                    //2021.10.13为完善义注连接，改动读经窗口里的代码，三个按钮计三处。
                    frmw.htmfilename = trNx.Name;
                    frmw.selfchaptermark = tsbtntika_zj;
                    frmw.selfparanum = tsbtntika_paranum;
                    frmw.isfromMAT = 1;
                    //
                    frmw.mulafile = ((tvtag)(trNx.Tag)).fnmula;
                    frmw.atthafile = ((tvtag)(trNx.Tag)).fnattha;
        ...
        {
            MessageBox.Show("若在词典窗口中选择了何种‘输入输出字体编码’，则在读经窗口中\r\n所复制输出的巴利文本即为何种字体编码的文本。", "关于复制：");
        }
        ...
        /// 存储义注连接标记的章节标记
        /// </summary>
        public ArrayList alKhantis = new ArrayList();

        /// <summary>
        /// 存储义注连接标记的段标记
        /// </summary>
        public ArrayList alNewparamarks = new ArrayList();

        /// <summary>
        /// 对于从mula/attha/tika打开的读经窗口，这个变量存储父窗口当前段所对应的章节的所有段id标记的段号数字部分（包括减号，如果有减号的话。）
        /// </summary>
        public ArrayList alChapterParas = new ArrayList();
...
public void getPTSpage(int iPTSMaxPage, ArrayList alPTSPages, string tsslName)
{
    ...
    catch (Exception e)
    {
        ...
        //取得全部义注连接定位标记khanti和newparamark类元素
        public void qdallClassKhantiNewparamark()
        {
            HtmlElementCollection tagA = webBrowser1.Document.GetElementsByTagName("a");
            HtmlElementCollection tagSpan = webBrowser1.Document.GetElementsByTagName("span");

            foreach (HtmlElement heA in tagA)
            {
                if (heA.GetAttribute("className") == "khanti")
                {
                    //ArrayList使用前必须用new初始化，否则运行会出错、而且不提示异常
                    alKhantis.Add(heA);
                }
            ...
            foreach (HtmlElement heB in tagSpan)
            {
                if (heB.GetAttribute("className") == "newparamark")
                {
                    //ArrayList使用前必须用new初始化，否则运行会出错、而且不提示异常
                    alNewparamarks.Add(heB);
                }
            }
        }

        //取得浏览器当前显示窗口中所显示可见的最上面一个段落的段Id
        public void getnewparamark()
        {
            if (alNewparamarks.Count <= 0)
            {
                return;
            }
            ...
            //浏览器里页面顶端的纵坐标是0，此Y值是元素距离页面顶端的距离，只要浏览器窗口宽度不变，页面宽度不变，无论页面如何滚动，它都不变
            //MessageBox.Show(((HtmlElement)alNewparamarks[1778]).OffsetRectangle.Y.ToString());
            //浏览器里页面顶端的纵坐标是0，此SCrollTop值是当前浏览器显示窗口的上边距离页面顶端的距离
            //MessageBox.Show(this.webBrowser1.Document.Body.ScrollTop.ToString());
            ...
            //最小偏移量段落的标号
            int minOffsetParaNo = -1;

            for (int p = 0; p < alNewparamarks.Count; p++)
            {
                currOffset = Math.Abs(((HtmlElement)alNewparamarks[p]).OffsetRectangle.Y - this.webBrowser1.Document.Body.ScrollTop);
            ...
            */
            ...
            int minId = 0;
            int maxId = alNewparamarks.Count - 1;
            int tmpId = (minId + maxId) / 2;

            while (tmpId > minId && tmpId < maxId)
            {
                if (((HtmlElement)alNewparamarks[tmpId]).OffsetRectangle.Y > this.webBrowser1.Document.Body.ScrollTop)
                {
                    maxId = tmpId;
                    tmpId = (minId + maxId) / 2;
                }
            ...
            {
                tmpId = maxId;
                if (((HtmlElement)alNewparamarks[minId]).OffsetRectangle.Y >= this.webBrowser1.Document.Body.ScrollTop)
                {
                    tmpId = minId;
                }
                if ((((HtmlElement)alNewparamarks[minId]).OffsetRectangle.Y < this.webBrowser1.Document.Body.ScrollTop) && (((HtmlElement)alNewparamarks[maxId]).OffsetRectangle.Y >= this.webBrowser1.Document.Body.ScrollTop + this.webBrowser1.Height-44))
                {
                    tmpId = minId;
                }
            }

            string pmark = ((HtmlElement)alNewparamarks[tmpId]).Id.ToString();
            /////
            string zhangjiebiaoji = "";
            string duanhao = "";
            MatchCollection mc = new Regex("(?<c>.*)_para(?<p>.*)", RegexOptions.IgnoreCase).Matches(pmark);
            foreach (Match ma in mc)
            {
                //取得章节标记和段号
                zhangjiebiaoji = ma.Groups["c"].Value;
                duanhao = ma.Groups["p"].Value;
                //this.Text = paliname + "#" + htmfilename + "#" + zhangjiebiaoji + "#" + duanhao;
                this.Text = paliname + "    #    curr para num : " + duanhao;

                for (int q = 0; q < 586; q++)
                {
                    if (Program.paramark[q, 0] == htmfilename + ".htm" && Program.paramark[q, 1] == zhangjiebiaoji)
                    {
                        if (Program.paramark[q, 2].Substring(Program.paramark[q, 2].Length - 8) == ".mul.htm")
                        {
                            tsbtnmula_htmfn = Program.paramark[q, 2];
                            tsbtnmula_zj = Program.paramark[q, 3];
                            tsbtnmula_paranum = duanhao;

                            tsbtnmula.Enabled = true;
                        }

                        if (Program.paramark[q, 2].Substring(Program.paramark[q, 2].Length - 8) == ".att.htm")
                        {
                            tsbtnattha_htmfn = Program.paramark[q, 2];
                            tsbtnattha_zj = Program.paramark[q, 3];
                            tsbtnattha_paranum = duanhao;

                            tsbtnattha.Enabled = true;
                        }

                        if (Program.paramark[q, 2].Substring(Program.paramark[q, 2].Length - 8) == ".tik.htm")
                        {
                            tsbtntika_htmfn = Program.paramark[q, 2];
                            tsbtntika_zj = Program.paramark[q, 3];
                            tsbtntika_paranum = duanhao;

                            tsbtntika.Enabled = true;
                        }
                    }
                }
            }
        ...
        public void htmonload()
        {
            ...
            qdallClassKhantiNewparamark();

            qddqpage();

            //此句滚动页面，qddqpage()再执行一次
        ...
        public void htmonload1()
        {
            ...
            //如果是从根本/义注/复注三个按钮所打开的，就进行义注段落自动定位跳转
            if (this.isfromMAT == 1)
            {
                string pmark = "";
                foreach (HtmlElement hepara in alNewparamarks)
                {
                    pmark = hepara.GetAttribute("id");
                    if (pmark.Substring(0, selfchaptermark.Length) == selfchaptermark)
                    {
                        alChapterParas.Add(pmark.Substring(selfchaptermark.Length + 5));
                    }
                }
                ...
                string a = "", b = "";
                a = selfparanum;
                for (int v = 0; v < alChapterParas.Count; v++)
                {
                    b = alChapterParas[v].ToString();
                    //a代表待定位段号
                    //b表示本for循环中的当前段号
                    if (v == 0)
                    {
                        //如果a小于或等于b，就是b，就返回b，结束循环
                        if (compareParanum(a, b) == "小于" || compareParanum(a, b) == "等于")
                        {
                            thepara = b;
                            break;
                        }
                        //如果a大于b，就继续，什么都不做
                        if (compareParanum(a, b) == "大于")
                        {
                        }

                        //MessageBox.Show(alChapterParas[v].ToString());
                    }

                    if (v > 0 && v < alChapterParas.Count - 1)
                    {
                        //如果a小于b，就是前一个，就返回前一个，结束循环
                        if (compareParanum(a, b) == "小于")
                        {
                            thepara = alChapterParas[v - 1].ToString();
                            break;
                        }
                        //如果a等于b，就是b，就返回b，结束循环
                        if (compareParanum(a, b) == "等于")
                        {
                            thepara = b;
                            break;
                        }
                    ...
                    if (v == alChapterParas.Count - 1)
                    {
                        ...
                        //如果a等于或大于b，就是b，就返回b，结束循环
                        if (compareParanum(a, b) == "等于" || compareParanum(a, b) == "大于")
                        {
                            thepara = b;
                            break;
                        }
                ...
                //selfparanum
                //((frmpali)(frmpali.FromHandle((IntPtr)(Form1.frmpalihandle)))).webBrowser1.Document.GetElementById(((frmmulu)(frmmulu.FromHandle((IntPtr)(Form1.frmmuluhandle)))).tnname).ScrollIntoView(true);
                ((frmpali)(frmpali.FromHandle((IntPtr)(Form1.frmpalihandle)))).webBrowser1.Document.GetElementById(selfchaptermark + "_para" + thepara).ScrollIntoView(true);
            }

            /*////test
            ((frmpali)(frmpali.FromHandle((IntPtr)(Form1.frmpalihandle)))).webBrowser1.Document.GetElementById("a2").ScrollIntoView(true);
            qdallClassKhantiNewparamark();
            MessageBox.Show(alKhantis.Count.ToString());
            MessageBox.Show(alNewparamarks.Count.ToString());
            MessageBox.Show(((HtmlElement)alKhantis[1]).Id);
            MessageBox.Show(((HtmlElement)alNewparamarks[190]).Id);
            ((frmpali)(frmpali.FromHandle((IntPtr)(Form1.frmpalihandle)))).webBrowser1.Document.GetElementById(((HtmlElement)alNewparamarks[190]).Id).ScrollIntoView(true);
            ////*/
        ...
        /// <summary>
        /// 比较段号大小，此段号形如：158，123-236包括减号
        /// </summary>
        public string compareParanum(string a, string b)
        {
            int ia, ib;
            //如果a，b包含减号，则取其减号前的数字进行比较
            string aParaNumHead = a;
            string bParaNumHead = b;
            MatchCollection mc = new Regex("(?<head>.*)-.*", RegexOptions.None).Matches(a);
            foreach (Match ma in mc)
            {
                aParaNumHead = ma.Groups["head"].Value;
            }

            MatchCollection mc2 = new Regex("(?<head>.*)-.*", RegexOptions.None).Matches(b);
            foreach (Match ma2 in mc2)
            {
                bParaNumHead = ma2.Groups["head"].Value;
            }

            ia = Convert.ToInt32(aParaNumHead);
            ib = Convert.ToInt32(bParaNumHead);
        ...
        private void copywithFormatonlyUnicodeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            Form1._isPaliWindowCopy = true;
